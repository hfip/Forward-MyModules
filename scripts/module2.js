...
async function fetchData(api, params, forceMediaType) {
  ...
  try {
    ...
    return response.results.map((item) => {
      ...
      return {
        id: `${mediaType}.${item.id}`, // ⚠️ صياغة المعرف القياسية لـ TMDB المطلوبة في التوثيق
        type: "tmdb",
        title: finalTitle,
        ...
        link: shahidDirectUrl // التوثيق يذكر استخدام 'link' لتمرير الروابط الخارجية لصفحات التفاصيل
      };
    });
  } catch (error) { 
    return []; 
