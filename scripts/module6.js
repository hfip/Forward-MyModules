...
// Module Name: VOD Portal — مكتبة الأفلام والمسلسلات 🎥
// Developer: Dex
// Telegram: @a6ahd
...
var WidgetMetadata = {
    id: "forward.dex.vod.portal.official",
    title: "مكتبة الأفلام والمسلسلات 🎥",
    detailCacheDuration: 3600, 
    version: "1.3.1",
    requiredVersion: "0.0.1",
    description: "جلب وتصفح مكتبة الأفلام والمسلسلات (VOD) من خوادم Xtream Codes مع ميزة تصفية وتصنيف الباقات من تطوير المطور Dex.",
    author: "Dex",
            ...
            params: [
                {
                    ...
                    type: "input",
                    description: "ادخل رابط السيرفر (مثال: http://example.com:8080)",
                    placeholders: [
                ...
                },
                {
                    ...
                    title: "نوع المحتوى",
                    type: "enumeration",
                    description: "اختر جلب مكتبة الأفلام أو مكتبة المسلسلات",
                    value: "movies", 
                    enumOptions: [
                        {title: "🎬 الأفلام", value: "movies"},
                        {title: "📺 المسلسلات", value: "series"},
                    ]
                },
                {
                    name: "category_filter",
                    title: "تصفية حسب الفئة / التصنيف (اختياري)",
                    type: "input",
                    description: "اختر تصنيفاً من القائمة الموصى بها أو اكتب تصنيفاً مخصصاً",
                    placeholders: [
                        { title: "الأحدث المضاف حديثاً", value: "" }, 
                ...
                {
                    name: "name_filter",
                    title: "البحث عن عنوان معين (اختياري)",
                    type: "input",
                    description: "اكتب اسم الفيلم أو المسلسل للبحث السريع عنه",
                ...
                },
                {
                    ...
                    title: "ترتيب النتائج",
                    type: "enumeration",
                    description: "اختر طريقة ترتيب عرض المواد",
                    value: "default",
                    enumOptions: [
                        { title: "افتراضي (ترتيب السيرفر الأصلي)", value: "default" },
                        { title: "ترتيب أبجدي (A-Z)", value: "az" },
                        { title: "ترتيب عكسي (Z-A)", value: "za" },
                        { title: "المضاف حديثاً (حسب التاريخ التنازلي)", value: "newest" }
                    ]
                }
            ],
        },
...
function cleanTitle(title) {
  if (!title) return "عنوان غير معروف";
  return title
    ...
    try {
        ...
        if (sortBy === 'az') {
            ...
            const getTimestamp = (item) => {
                let ts = 0;
                if (type === 'movies') ts = item.metadata?.added;
                else if (type === 'series') ts = item.metadata?.last_modified;
...
function parseVodContent(data, type, portal, username, password, categoryMap) {
    ...
    for (const item of data) {
        ...
        if (widgetItem) items.push(widgetItem);
    }
    return items;
}
...
async function loadDetail(link) {
    try {
        ...
        if (parts[0] !== 'xtream') throw new Error("Invalid format");
        ...
        if (type === 'movie') {
            ...
            if (!movieData) throw new Error("Failed info load");
            ...
            return {
                id: `movie_detail_${id}`,
                type: "detail",
                title: cleanTitle(info.name) || "فيلم",
                description: info.plot || "لا يوجد وصف متاح لهذا الفيلم.",
            ...
            if (!seriesData) throw new Error("Failed info load");
            ...
            const addEpisode = (ep, seasonNum, safeEpisodeCounter) => { 
                ...
                if (!firstEpisodeUrl) firstEpisodeUrl = epLink;
            ...
            return {
                ...
                description: info.plot || "لا يوجد وصف متاح لهذا المسلسل.",
